...
Fixes mypy type errors and eliminates `Any` usage in the sbclaude project.
...
1. Fix all mypy errors reported by `uv run mypy sbclaude`
...
Place new TypedDict classes in `sbclaude/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`sbclaude/typing.py` when shared.
...
1. Run `uv run mypy sbclaude` and capture all errors.
...
4. Run `uv run mypy sbclaude` again to verify fixes.
