...
If any changed files are under `sbclaude/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `sbclaude/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `sbclaude/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `sbclaude/commands/main.py`,
...
For Python files, strip the `sbclaude/` prefix and replace `/` with `.` (like module imports).

- Python file `sbclaude/media.py` → `media:`.
- Multiple files under `sbclaude/commands/` → `commands:`.
- Single command file `sbclaude/commands/admin.py` → `commands.admin:`.
